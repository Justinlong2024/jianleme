...
const DOC_CONTENT = `# 简了么 (JianLeMa) — 微信小程序开发者完整文档
...
const DocViewerPage = () => {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText(DOC_CONTENT);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  const handleDownload = () => {
    const blob = new Blob([DOC_CONTENT], { type: "text/markdown;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "WECHAT_MINIPROGRAM_SPEC.md";
    a.click();
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-6 sticky top-0 bg-background py-4 z-10 border-b border-border">
          <h1 className="text-xl font-bold text-foreground flex-1">📄 微信小程序开发文档</h1>
          <button
            onClick={handleCopy}
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-secondary text-secondary-foreground hover:opacity-90 transition-opacity text-sm font-medium"
          >
            <Download className="w-4 h-4" />
            下载 .md
          </button>
        </div>
        <pre className="whitespace-pre-wrap text-sm text-foreground font-mono bg-card rounded-2xl p-6 shadow-sm leading-relaxed overflow-auto">
          {DOC_CONTENT}
        </pre>
      </div>
    </div>
